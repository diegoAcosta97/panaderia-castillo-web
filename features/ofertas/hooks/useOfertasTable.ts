...
import { listOfertasPaginated } from "@/repositories/ofertasRepository";
import type { OfertaConItems } from "@/repositories/ofertasRepository";

const PAGE_SIZE = 20;

export function useOfertasTable() {
  const [pageIndex, setPageIndex] = useState(0);
  const [sorting, setSorting] = useState<SortingState>([]);
  const [data, setData] = useState<OfertaConItems[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const sort = sorting[0]
        ...
        : undefined;
      const result = await listOfertasPaginated(supabase, {
        page: pageIndex,
        pageSize: PAGE_SIZE,
        sort,
      });
