-- E7-11: cargar/editar un renglón "por peso" por monto (docs/backlog/07-punto-de-venta.md) --
-- el peso se redondea al gramo para poder guardarse (numeric(12,3)), lo que casi nunca reproduce
-- el monto tipeado exacto a precio de catálogo (ej. "$1000 de pan malteado" a $3500/kg redondea a
-- 0,286 kg, que a precio de lista son $1001, no $1000). El cliente (Carrito/PesoDialog,
-- services/beneficiosService.ts no interviene acá) ahora puede mandar un `precio_unitario` por
-- renglón, ajustado para que cantidad * precio_unitario dé justo el monto pedido -- esta función
-- lo valida contra una tolerancia chica antes de aceptarlo, para no confiar ciegamente en un
-- precio mandado por el cliente (mismo criterio que la validación de "medios de pago == total").
--
-- La tolerancia es en pesos, no en % del precio: el error que puede introducir redondear el peso
-- al gramo más cercano es como mucho medio gramo de mercadería, así que se acota a
-- greatest(precio_catalogo * 0.001, 0.01) -- aprox. el valor de un gramo, con un piso de 1
-- centavo -- multiplicado por la cantidad de la línea. No alcanza para forzar un descuento real:
-- para un producto de $3500/kg, permite ajustar como mucho ~$3,50 sobre el total de esa pesada,
-- sea cual sea el peso.
--
-- Agrupar los renglones para armar `renglones_venta` pasa de ser por `producto_id` solo a ser
-- por `(producto_id, precio_unitario)`: dos pesadas del mismo producto a distinto precio efectivo
-- (una a catálogo, otra ajustada "por monto") tienen que quedar en filas separadas -- NULL
-- agrupa junto con NULL en SQL estándar, así que el caso común (sin ningún override) se sigue
-- agrupando en una sola fila por producto, igual que antes. La validación de stock (loop propio,
-- sin cambios) sigue agrupando solo por producto_id -- el precio no afecta cuánto stock se
-- descuenta.
