...
-- registrar_merma -- mismo esqueleto (bloquea, resta stock_actual, inserta movimiento), mismo
-- riesgo de NULL - cantidad = NULL pisando el NOT NULL de stock_resultante.
...
begin
  ...
  if v_producto.stock_actual is null then
    raise exception
      '"%" controla stock pero no tiene stock inicial cargado. Corregilo desde el catálogo de productos antes de registrar una merma.',
      v_producto.nombre;
  end if;
