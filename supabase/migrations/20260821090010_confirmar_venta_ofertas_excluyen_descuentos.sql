-- E6-6: ofertas y descuentos dejan de acumularse (docs/backlog/06-ofertas-descuentos.md,
-- decisión que reemplaza a RF-3.6) -- regla nueva pedida por el dueño: la oferta se aplica
-- siempre y puede haber más de una por venta; el descuento solo se aplica si la venta no tiene
-- ninguna oferta aplicada. `services/beneficiosService.ts` (evaluarBeneficios) ya no evalúa
-- descuentos cuando hay ofertas, así que en el flujo normal del punto de venta nunca llega acá
-- un p_descuentos no vacío junto con un p_ofertas no vacío -- este guard es la misma regla
-- reforzada server-side (mismo criterio que la validación de "medios de pago == total" de acá
-- abajo: no confiar únicamente en el cálculo del cliente para una regla de negocio).
