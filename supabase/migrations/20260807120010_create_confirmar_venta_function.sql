-- E7-3: confirmación transaccional de venta (docs/backlog/07-punto-de-venta.md)
--
-- Todo en una sola transacción: bloquea (`for update`) cada producto involucrado, valida stock
-- suficiente para los que lo controlan, inserta venta + renglones + beneficios + medios de
-- pago, y descuenta stock (con su movimiento) solo si la venta queda `completada`. Si algo
-- falla, no se descuenta ni se inserta nada (rollback automático de la función).
--
-- Estado de la venta (coincide con docs/data-model.md#ventas): `completada` si todos los
-- medios de pago son efectivo; `pendiente_pago` si hay algún medio `mercado_pago` — en ese caso
-- el stock **no** se descuenta acá, queda para cuando EPIC 8 confirme el pago (función a
-- definir en esa epic, ej. `confirmar_pago_venta`).
--
-- `for update` sobre `productos` es lo que evita la condición de carrera: dos llamadas
-- concurrentes a esta función para el mismo producto se serializan en esa fila (la segunda
-- espera a que la primera termine su transacción), así nunca leen un stock_actual desactualizado.
...
begin
  ...
  --    producto_id: si el carrito trae el mismo producto en más de un renglón (ej. una balanza
  --    pesada dos veces), se valida/descuenta por el total combinado -- si no, dos renglones
  --    del mismo producto pasarían la validación cada uno por separado contra el mismo stock,
  --    sin ver el descuento implícito del otro.
  for v_renglon in
  ...
  -- 2) Renglones (uno por producto, ya agrupado) + descuento de stock (solo si queda completada).
  for v_renglon in
    select producto_id, sum(cantidad) as cantidad
    from jsonb_to_recordset(p_renglones) as x(producto_id uuid, cantidad numeric)
    group by producto_id
  loop
    ...
    insert into public.renglones_venta (venta_id, producto_id, cantidad, precio_unitario_snapshot, subtotal)
    values (v_venta_id, v_renglon.producto_id, v_renglon.cantidad, v_producto.precio,
      v_renglon.cantidad * v_producto.precio);
