-- Trata 'sena_pedido' igual que 'efectivo' a los fines de dar la venta por completada y marcar
-- ese medio como acreditado de inmediato -- sin esto, una venta efectivo + sena_pedido quedaría
-- en pendiente_pago esperando un webhook de Mercado Pago que nunca llega (docs: despacho de
-- pedidos por encargo).
...
begin
  ...
  select case
    when bool_and(x ->> 'medio_pago' in ('efectivo', 'sena_pedido')) then 'completada'::public.estado_venta
  ...
  select
    ...
    (x ->> 'monto')::numeric,
    case when (x ->> 'medio_pago') in ('efectivo', 'sena_pedido')
