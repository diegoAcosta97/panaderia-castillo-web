-- Guarda defensiva: si por algún motivo un producto con controla_stock = true llegara a tener
-- stock_actual NULL (el constraint agregado en 20260822090000 ya lo impide a nivel de datos, pero
-- esto es defensa en profundidad), que confirmar_venta lo rechace con un mensaje entendible en vez
-- de dejar pasar la venta y romper en el insert de movimientos_stock (NOT NULL de
-- stock_resultante) -- ver esa migración para el detalle del bug original.
...
begin
  ...
  loop
    ...
    if v_producto.controla_stock and v_producto.stock_actual is null then
      raise exception
        '"%" controla stock pero no tiene stock inicial cargado. Corregilo desde el catálogo de productos antes de vender.',
        v_producto.nombre;
    end if;
